...
from app.postgres_models import Branch, Department, Employee, StoreZone
...
async def prometheus_metrics_middleware(request: Request, call_next):
    # Не считаем сам /metrics, чтобы не раздувать метрики.
    ...
    if route is not None and hasattr(route, "path"):
        # FastAPI дает шаблон роутов вида "/products/{category_id}/products",
        # что ограничивает кардинальность меток.
        path = route.path
    ...
    HTTP_REQUEST_LATENCY_SECONDS.labels(
        method=request.method,
        path=path,
    ).observe(pytime.perf_counter() - start)
    return response
...
@app.get("/auth/me")
async def auth_me(user: Annotated[AuthUser, Depends(get_current_user)]) -> dict[str, str]:
...
async def livez() -> dict[str, str]:
    # Liveness probe: процесс запущен (без обращения к БД).
    return {"status": "ok"}


@app.get("/readyz")
async def readyz(db: Annotated[AsyncIOMotorDatabase, Depends(database)]) -> dict[str, str]:
    # Readiness probe: приложение способно обрабатывать запросы (БД отвечает).
    await db.command("ping")
    return {"status": "ok", "database": db.name}
...
async def list_categories(
    ...
    _auth=Depends(require_roles("admin", "manager", "user", "guest")),
) -> list[CategoryOut]:
...
async def list_customers(
    db: Annotated[AsyncIOMotorDatabase, Depends(database)],
    _auth=Depends(require_roles("admin", "manager", "user")),
) -> list[CustomerOut]:
...
async def create_product(
    ...
    db: Annotated[AsyncIOMotorDatabase, Depends(database)],
    _auth=Depends(require_roles("admin", "manager")),
) -> ProductOut:
...
async def get_customer_cart(
    customer_id: str,
    db: Annotated[AsyncIOMotorDatabase, Depends(database)],
    _auth=Depends(require_roles("admin", "user")),
) -> CartOut:
...
async def _get_or_create_cart(db: AsyncIOMotorDatabase, customer_oid: ObjectId) -> dict[str, Any]:
    cart = await db.carts.find_one({"customerId": customer_oid})
    if cart is None:
        cart = {"customerId": customer_oid, "products": [], "updatedAt": now_utc()}
        await db.carts.insert_one(cart)
...
async def _save_cart(db: AsyncIOMotorDatabase, customer_oid: ObjectId, cart: dict[str, Any]) -> CartOut:
    cart["updatedAt"] = now_utc()
    await db.carts.replace_one({"customerId": customer_oid}, cart, upsert=True)
    return serialize_cart(cart)


@app.patch("/customers/{customer_id}/cart/items/{product_id}", response_model=CartOut)
async def update_cart_item(
    ...
    cart = await _get_or_create_cart(db, customer_oid)
    products = cart.get("products", [])
    found = False
    for cart_item in products:
        if cart_item["productId"] == product_oid:
    ...
    cart["products"] = products
    return await _save_cart(db, customer_oid, cart)


@app.delete("/customers/{customer_id}/cart/items/{product_id}", response_model=CartOut)
async def remove_cart_item(
    ...
    cart = await _get_or_create_cart(db, customer_oid)
    products = cart.get("products", [])
    new_products = [item for item in products if item["productId"] != product_oid]
...
async def get_employees(
    ...
    return [_employee_out(e) for e in result.scalars().all()]
...
async def create_employee(
    ...
    new_employee = Employee(
        ...
        hired_at=employee.hired_at or date.today(),
    )

    db.add(new_employee)
    await db.commit()
    await db.refresh(new_employee)
    return _employee_out(new_employee)
...
async def get_branches(
    ...
    return [_branch_out(b) for b in result.scalars().all()]
...
async def create_branch(
    ...
    return _branch_out(new_branch)


@app.get("/branches/{branch_id}/structure", response_model=BranchStructureOut)
async def get_branch_structure(
    ...
    departments = (
        await db.execute(select(Department).where(Department.branch_id == branch_id))
    ).scalars().all()
    zones = (
        await db.execute(select(StoreZone).where(StoreZone.branch_id == branch_id))
    ).scalars().all()
    employees = (
        await db.execute(select(Employee).where(Employee.branch_id == branch_id))
    ).scalars().all()

    return BranchStructureOut(
        branch=_branch_out(branch),
        departments=[
            DepartmentOut(id=d.id, branch_id=d.branch_id, name=d.name, description=d.description)
        ...
        store_zones=[
            StoreZoneOut(
                ...
                area_sqm=float(z.area_sqm) if z.area_sqm is not None else None,
            )
            for z in zones
        ],
        employees=[_employee_out(e) for e in employees],
    )
...
async def create_department(
    ...
    return DepartmentOut(id=dept.id, branch_id=dept.branch_id, name=dept.name, description=dept.description)


@app.get("/store-zones", response_model=list[StoreZoneOut])
async def list_store_zones(
    db: Annotated[AsyncSession, Depends(postgres_db)],
    branch_id: int | None = Query(default=None),
    _auth=Depends(require_roles("admin", "manager")),
):
    stmt = select(StoreZone)
    if branch_id is not None:
        stmt = stmt.where(StoreZone.branch_id == branch_id)
    rows = (await db.execute(stmt)).scalars().all()
    return [
        StoreZoneOut(
            id=z.id,
...
@app.post("/store-zones", response_model=StoreZoneOut)
async def create_store_zone(
    ...
    return StoreZoneOut(
        ...
        area_sqm=float(zone.area_sqm) if zone.area_sqm is not None else None,
    )